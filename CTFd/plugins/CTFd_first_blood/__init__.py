...
class FirstBloodValueChallenge(BaseChallenge):
    ...
    def update(cls, challenge, request):
        ...
        # Handle first_blood_bonus explicitly
        first_blood_bonus = None
        ...
        if first_blood_bonus is not None:
            for i in itertools.count():
                attr = f'first_blood_bonus[{i}]'
                if attr not in data:
                    break
                first_blood_bonus.append(int(data[attr]) if data[attr] != '' else None)
            while first_blood_bonus and first_blood_bonus[-1] is None:
                first_blood_bonus.pop()
            setattr(challenge, 'first_blood_bonus', first_blood_bonus)

        # Fix: explicitly clear missing optional fields
        # For example: if 'description' or 'connection_info' are not present, clear them
        for field in ['description', 'connection_info', 'attribution']:
            if field not in data:
                setattr(challenge, field, '')
    ...
    def _gen_award_data(cls, challenge, solve, solve_num):
        ...
        return {
            ...
            'name': '{0} Blood - {1}'.format(ordinalize(solve_num), challenge.name),
            'description': 'Recognition for securing the {0} solve of the challenge.'.format(ordinalize(solve_num)),
    ...
    def solve(cls, user, team, challenge, request):
        ...
        if FirstBloodValueChallenge._can_get_award(challenge, solve):
            ...
            if award_data is not None:
                ...
                db.session.commit()  # Add this commit!
...
def after_bulk_delete(delete_context):
    # Fix: Check if the delete context has the attributes we need
    try:
        # Try to get the table name from different possible attributes
        table_name = None
        if hasattr(delete_context, 'primary_table') and delete_context.primary_table:
            table_name = delete_context.primary_table.name
        elif hasattr(delete_context, 'table') and delete_context.table:
            table_name = delete_context.table.name
        elif hasattr(delete_context, 'mapper') and delete_context.mapper:
            table_name = delete_context.mapper.class_.__tablename__
        
        if table_name == "solves":
            # A batch delete of solves just occurred
        ...
        # If we can't determine the table name, just ignore this event
        # This prevents the plugin from crashing when bulk deletes occur on other tables
        pass
    ...
    for instance in session.dirty:
        if session.is_modified(instance):
            if isinstance(instance, db.Model):
                if hasattr(instance, "hidden") and hasattr(instance, "banned"):
